(function () {
	...
	RS.prototype.setInitialValues = function () {
		...
		return this.createScale(null, true);
	};

	RS.prototype.createScale = function (resize, initial=false) {
		...
		return this.addEvents(initial);
	};

	RS.prototype.updateScale = function (initial=false) {
		this.step = this.sliderWidth / (this.conf.values.length - 1);
		...
		return this.setValues(false, false, initial);
	};

	RS.prototype.addEvents = function (initial=false) {
	...
	RS.prototype.setValues = function (start, end, initial=false) {
        // TODO: variable name re-use is confusing
        ...
        // only modify the left pointer's style if we are moving the left pointer
		...
		if (this.conf.range) {
            // only change the tooltip of the side of the slider that is being dragged
			if (this.conf.tooltip && initial || this.activePointer == this.pointerL) {
				this.tipL.innerHTML = this.conf.values[this.values.start];
			}
            if (this.conf.tooltip && initial || this.activePointer == this.pointerR) {
				this.tipR.innerHTML = this.conf.values[this.values.end];
                console.log(this.tipR.innerHTML);
            }

			this.input.value = this.conf.values[this.values.start] + ',' + this.conf.values[this.values.end];

            // only redraw this if the active pointer is the right one
